...
_COND_UNSET = object()  # Sentinel to distinguish f(x) from f(x, cond=None)
...
class GemmaRMSNorm(nn.Module):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        # Auto-create dense layer if checkpoint has dense weights but we don't have it
        dense_weight_key = prefix + "dense.weight"
        dense_bias_key = prefix + "dense.bias"
        if dense_weight_key in state_dict and self.dense is None:
            dense_weight = state_dict[dense_weight_key]
            out_features, in_features = dense_weight.shape
            self.dense = nn.Linear(in_features, out_features, bias=dense_bias_key in state_dict)
            self.cond_dim = in_features
            # Remove self.weight since we're switching to adaptive mode
            if hasattr(self, 'weight'):
                del self.weight
        super()._load_from_state_dict(state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs)

    def forward(self, x, cond=_COND_UNSET):
        dtype = x.dtype
        normed_inputs = self._norm(x.float())

        if cond is _COND_UNSET:
            # Standard Gemma call: norm(x) - single tensor return
            if hasattr(self, 'weight') and self.weight is not None:
                output = normed_inputs * (1.0 + self.weight.float())
            else:
                output = normed_inputs
            return output.type_as(x)

        # PI0.5 call: norm(x, cond=...) - always returns tuple
        if self.dense is None or cond is None:
            # No adaptive conditioning (paligemma path or no cond)
            if hasattr(self, 'weight') and self.weight is not None:
                output = normed_inputs * (1.0 + self.weight.float())
            else:
                output = normed_inputs
